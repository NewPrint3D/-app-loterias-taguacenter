...
// Resolve o telefone real (só dígitos) a partir de um JID do WhatsApp.
// Baileys v7 expõe o mapeamento LID→número: participantes com privacidade ativada (@lid),
// que antes vinham SEM telefone nenhum, agora têm o número real acessível — ou pelo campo
// phoneNumber que acompanha o participante no groupMetadata, ou consultando o lidMapping.
// `pnCandidato` é esse campo phoneNumber/jid alternativo quando a fonte do evento fornecer.
async function foneDoJid(jid, pnCandidato) {
  try {
    let pn = '';
    if (pnCandidato && String(pnCandidato).includes('@s.whatsapp.net')) pn = String(pnCandidato);
    if (!pn && jid && jid.endsWith('@s.whatsapp.net')) pn = jid;
    if (!pn && jid && jid.endsWith('@lid') && botSock?.signalRepository?.lidMapping?.getPNForLID) {
      pn = await botSock.signalRepository.lidMapping.getPNForLID(jid) || '';
    }
    return pn ? String(pn).split('@')[0].split(':')[0].replace(/\D/g, '') : '';
  } catch { return ''; }
}
...
async function conectarBot() {
  try {
    ...
    botSock = makeWASocket({
      ...
      // Sincroniza o histórico/contatos na conexão — é daí que vêm os NOMES de exibição dos
      // participantes (groupMetadata não devolve nome em nenhuma versão do Baileys; antes o
      // nome só era capturado quando a pessoa mandava mensagem nova no grupo).
      syncFullHistory: true,
    });

    botSock.ev.on('creds.update', saveCreds);

    // ---- LISTENER: sincronização de contatos → preenche nomes/fones em grupo_membros ----
    // Dispara na conexão (messaging-history.set / contacts.upsert) e em atualizações.
    // Só sobrescreve nome vazio/"Sem nome" — preserva correção manual do admin.
    const atualizarNomesContatos = async contatos => {
      for (const c of (contatos || [])) {
        try {
          const nome = (c.notify || c.name || c.verifiedName || '').trim();
          const jids = [c.id, c.lid, c.jid].filter(Boolean);
          if (!jids.length) continue;
          if (nome) {
            await pool.query(
              `UPDATE grupo_membros SET nome=$1 WHERE wpp_jid = ANY($2)
               AND (nome='' OR LOWER(nome)='sem nome')`,
              [nome, jids]
            );
          }
          // Se o contato traz o par LID + número real, aproveita pra preencher fone vazio
          const fone = await foneDoJid(c.id, c.jid);
          if (fone) {
            await pool.query(
              `UPDATE grupo_membros SET fone=$1 WHERE wpp_jid = ANY($2) AND fone=''`,
              [fone, jids]
            );
          }
        } catch (e) { console.error('Erro ao sincronizar contato:', e.message); }
      }
    };
    botSock.ev.on('contacts.upsert', atualizarNomesContatos);
    botSock.ev.on('contacts.update', atualizarNomesContatos);
    botSock.ev.on('messaging-history.set', ({ contacts }) => atualizarNomesContatos(contacts));
    ...
    botSock.ev.on('messages.upsert', async ({ messages, type }) => {
      ...
      for (const msg of messages) {
        try {
          ...
          // v7: quando o remetente é @lid, o número real pode vir em key.participantPn —
          // e se não vier, foneDoJid ainda tenta o lidMapping.
          const foneReal = await foneDoJid(senderJid, msg.key.participantPn);
          ...
          if (msg.pushName) {
            try {
              ...
              if (gRows.length) {
                ...
                // Só sobrescreve nome vazio/"Sem nome" e fone vazio — preserva correção manual.
                await pool.query(
                  ...
                  [crypto.randomUUID(), grupoId, msg.pushName, foneReal, senderJid, new Date().toISOString().split('T')[0]]
                );
              }
    ...
    botSock.ev.on('group-participants.update', async ({ id: groupJid, participants, action }) => {
      try {
        ...
        for (const p of participants) {
          // v7 pode entregar objeto {id, phoneNumber} em vez de string simples
          const jid = typeof p === 'string' ? p : p.id;
          const pn  = typeof p === 'string' ? '' : (p.phoneNumber || p.jid || '');
          if (action === 'add') {
            const fone = await foneDoJid(jid, pn);
            await pool.query(
    ...
    botSock.ev.on('connection.update', async upd => {
      ...
      if (connection === 'close') {
        const { DisconnectReason: DR } = require('baileys');
...
// Importar". Com Baileys v7, participantes @lid (privacidade) vêm com o campo phoneNumber
// (número real) — antes vinham sem telefone nenhum. NOMES continuam não vindo no groupMetadata
// (limitação do WhatsApp em qualquer versão): chegam pela sincronização de contatos na conexão
// (listener contacts/messaging-history acima) ou quando a pessoa manda mensagem (pushName).
// Participante que já existia (mesmo wpp_jid) é reativado e tem o fone preenchido se estava
// vazio — nome nunca é sobrescrito (preserva edição manual ou pushName já capturado).
async function importarParticipantesGrupo(grupoId, wppJid) {
  ...
  for (const p of meta.participants) {
    const fone = await foneDoJid(p.id, p.phoneNumber || p.jid);
...
app.get('/api/wpp/participantes/:jid', async (req, res) => {
  ...
  try {
    ...
    const participantes = [];
    for (const p of meta.participants) {
      // Baileys v7: participante @lid (privacidade) traz o número real no campo phoneNumber
      // (ou resolvível via lidMapping). foneOculto só fica true se NEM ASSIM o número aparecer
      // — nesse caso o campo continua editável no modal pro admin preencher à mão.
      const fone = await foneDoJid(p.id, p.phoneNumber || p.jid);
      participantes.push({
        jid: p.id,
        fone,
